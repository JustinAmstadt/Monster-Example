...
import { printSchema as t } from "graphql";

import { load as i, outputIntrospectionFile as s, minifyIntrospection as F, resolveTypeScriptRootDir as C } from "@gql.tada/internal";
...
class o {
  constructor(u, D) {
    ...
    this.command(A), this.command([ E ].concat(D ? i : "<command>").join(" ")), this.single = D, 
    this.curr = "";
  }
  command(u, D, t = {}) {
    if (this.single) {
  ...
  describe(u) {
    return this.tree[this.curr || E].describe = Array.isArray(u) ? u : function(u) {
  ...
  option(u, D, t) {
    var i = this.tree[this.curr || A], [s, F] = function(u) {
  ...
  action(u) {
    return this.tree[this.curr || E].handler = u, this;
  }
  example(u) {
    return this.tree[this.curr || E].examples.push(u), this;
  }
  version(u) {
  ...
  parse(u, D = {}) {
    ...
    if (f) {
      ...
      var p, v = 1, g = c._.length + 1;
      for (;v < g; v++) {
        if (t = c._.slice(0, v).join(" "), "string" == typeof (p = this.tree[t])) {
          i = (h = p).split(" "), u.splice(u.indexOf(c._[0]), v, ...i), v += i.length - v;
        } else if (p) {
          h = t;
        } else if (h) {
          break;
      ...
      if (s = void 0 === (F = this.tree[h])) {
        if (this.default) {
          F = this.tree[h = this.default], u.unshift(h), C++;
        } else if (t) {
    ...
    var w = F.usage.split(/\s+/), b = w.filter((u => "<" === u.charAt(0))), x = y._.splice(0, b.length);
  ...
  help(u) {
    console.log(function(u, D, t, i) {
      var s = "", F = D[t], C = "$ " + u, c = D[A], d = u => `${C} ${u}`.replace(/\s+/g, " "), B = [ [ "-h, --help", "Displays this message" ] ];
      if (t === E && B.unshift([ "-v, --version", "Displays current version" ]), F.options = (F.options || []).concat(c.options, B), 
      F.options.length > 0 && (F.usage += " [options]"), s += l("Description", F.describe, n), 
      s += l("Usage", [ F.usage ], d), i || t !== E) {
        i || t === E || (s += l("Aliases", F.alibi, d));
      } else {
        var h, p = /^__/, v = "", g = [];
        for (h in D) {
          "string" == typeof D[h] || p.test(h) || g.push([ h, (D[h].describe || [ "" ])[0] ]) < 3 && (v += `\n    ${C} ${h} --help`);
        }
        s += l("Available Commands", r(g), n), s += "\n  For more info, run any command with the `--help` flag" + v + f;
      }
      return s += l("Options", r(F.options), n), s += l("Examples", F.examples.map(d), n);
    }(this.bin, this.tree, u || E, this.single));
  }
  _version() {
...
B.exports = function() {
  ...
  var U = {
    ...
    decimalInteger: function decimalInteger() {
      ...
      return newToken("numeric", H * Number(G));
    },
  ...
  function invalidChar(u) {
    if (void 0 === u) {
      return syntaxError("JSON5: invalid end of input at " + V + ":" + z);
    }
    return syntaxError("JSON5: invalid character '" + formatChar(u) + "' at " + V + ":" + z);
  }
  function invalidEOF() {
    return syntaxError("JSON5: invalid end of input at " + V + ":" + z);
  }
  function invalidIdentifier() {
    return syntaxError("JSON5: invalid identifier character at " + V + ":" + (z -= 5));
  }
...
async function generateSchema(s, {headers: F, output: C, cwd: c = process.cwd()}) {
  ...
  if (!f) {
    var B;
    try {
      B = await u.readFile("tsconfig.json", "utf-8");
    } catch (u) {
  ...
  await u.writeFile(D.resolve(c, f), t(E), "utf-8");
}

async function generateTadaTypes(t = !1, A = process.cwd()) {
  var E = D.resolve(A, "tsconfig.json");
  var f = await C(E) || A;
  var B;
  try {
    var p = D.resolve(f, "tsconfig.json");
    B = await u.readFile(p, "utf-8");
  } catch (u) {
    console.error("Failed to read tsconfig.json in current working directory.", u);
    return;
  }
  ...
  var g = getGraphQLSPConfig(v);
  if (!g) {
    console.error('Could not find a "@0no-co/graphqlsp" plugin in your tsconfig.');
    return;
  }
  return await async function ensureTadaIntrospection(u, t, C = process.cwd(), A = !0) {
    var E = i({
      origin: u,
      rootPath: C
    });
