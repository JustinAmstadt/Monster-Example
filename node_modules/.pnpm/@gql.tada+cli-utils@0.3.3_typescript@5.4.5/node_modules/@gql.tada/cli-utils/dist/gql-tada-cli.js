...
var u = require("node:fs/promises");
...
function e(u, D) {
  ...
  var h = void 0 !== (D = D || {}).alias;
  var p = void 0 !== D.unknown;
...
class o {
  constructor(u, D) {
    ...
    this.command(F), this.command([ C ].concat(D ? i : "<command>").join(" ")), this.single = D, 
    this.curr = "";
  }
  command(u, D, t = {}) {
    if (this.single) {
  ...
  describe(u) {
    return this.tree[this.curr || C].describe = Array.isArray(u) ? u : function(u) {
  ...
  option(u, D, t) {
    var i = this.tree[this.curr || F], [s, C] = function(u) {
      return (u || "").split(/^-{1,2}|,|\s+-{1,2}|\s+/).filter(Boolean);
    }(u);
    if (C && C.length > 1 && ([s, C] = [ C, s ]), u = "--" + s, C && C.length > 0) {
      u = `-${C}, ${u}`;
      i.alias[C] = (i.alias[C] || []).concat(s);
    }
    var c = [ u, D || "" ];
    return void 0 !== t ? (c.push(t), i.default[s] = t) : C || (i.default[s] = void 0), 
    i.options.push(c), this;
  }
  action(u) {
    return this.tree[this.curr || C].handler = u, this;
  }
  example(u) {
    return this.tree[this.curr || C].examples.push(u), this;
  }
  version(u) {
    return this.ver = u, this;
  }
  parse(u, D = {}) {
    var t, i, s, c, A = 2, E = e((u = u.slice()).slice(A), {
      alias: {
        h: "help",
        v: "version"
      }
    }), f = this.single, B = this.bin, h = "";
    if (f) {
      c = this.tree[C];
    } else {
      var p, v = 1, g = E._.length + 1;
      for (;v < g; v++) {
        if (t = E._.slice(0, v).join(" "), "string" == typeof (p = this.tree[t])) {
          i = (h = p).split(" "), u.splice(u.indexOf(E._[0]), v, ...i), v += i.length - v;
        } else if (p) {
          h = t;
        } else if (h) {
          break;
      ...
      if (s = void 0 === (c = this.tree[h])) {
        if (this.default) {
          c = this.tree[h = this.default], u.unshift(h), A++;
        } else if (t) {
          return a(B, "Invalid command: " + t);
        }
      }
    }
    if (E.help) {
      return this.help(!f && !s && h);
    }
    if (E.version) {
      return this._version();
    }
    if (!f && void 0 === c) {
      return a(B, "No command specified.");
    }
    var m = this.tree[F];
    D.alias = Object.assign(m.alias, c.alias, D.alias), D.default = Object.assign(m.default, c.default, D.default), 
    t = h.split(" "), ~(i = u.indexOf(t[0], 2)) && u.splice(i, t.length);
    var y = e(u.slice(A), D);
    if (!y || "string" == typeof y) {
      return a(B, y || "Parsed unknown option flag(s)!");
    }
    var w = c.usage.split(/\s+/), b = w.filter((u => "<" === u.charAt(0))), x = y._.splice(0, b.length);
    if (x.length < b.length) {
      return h && (B += " " + h), a(B, "Insufficient arguments!");
    }
  ...
  help(u) {
    console.log(function(u, D, t, i) {
      var s = "", A = D[t], E = "$ " + u, f = D[F], d = u => `${E} ${u}`.replace(/\s+/g, " "), B = [ [ "-h, --help", "Displays this message" ] ];
      if (t === C && B.unshift([ "-v, --version", "Displays current version" ]), A.options = (A.options || []).concat(f.options, B), 
      A.options.length > 0 && (A.usage += " [options]"), s += l("Description", A.describe, n), 
      s += l("Usage", [ A.usage ], d), i || t !== C) {
        i || t === C || (s += l("Aliases", A.alibi, d));
      } else {
        var h, p = /^__/, v = "", g = [];
        for (h in D) {
          "string" == typeof D[h] || p.test(h) || g.push([ h, (D[h].describe || [ "" ])[0] ]) < 3 && (v += `\n    ${E} ${h} --help`);
        }
        s += l("Available Commands", r(g), n), s += "\n  For more info, run any command with the `--help` flag" + v + c;
      }
      return s += l("Options", r(A.options), n), s += l("Examples", A.examples.map(d), n);
    }(this.bin, this.tree, u || C, this.single));
  }
  _version() {
...
A.exports = function() {
  ...
  var p = h("native-function-to-string", Function.toString);
  var v = createCommonjsModule((function(t) {
    ...
    (t.exports = function(D, t, s, C) {
      ...
      return "function" == typeof this && this[i] || p.call(this);
    }));
  }));
  ...
  var _toAbsoluteIndex = function(u, D) {
    return (u = _toInteger(u)) < 0 ? x(u + D, 0) : S(u, D);
  };
  var P = String.fromCharCode;
  var k = String.fromCodePoint;
  m(m.S + m.F * (!!k && 1 != k.length), "String", {
  ...
  function peek() {
    if (_[I]) {
      return String.fromCodePoint(_.codePointAt(I));
    }
  }
  function read() {
    var u = peek();
  ...
  var U = {
    ...
    decimalInteger: function decimalInteger() {
      ...
      return newToken("numeric", H * Number(R));
    },
  ...
  function invalidChar(u) {
    if (void 0 === u) {
      return syntaxError("JSON5: invalid end of input at " + q + ":" + V);
    }
    return syntaxError("JSON5: invalid character '" + formatChar(u) + "' at " + q + ":" + V);
  }
  function invalidEOF() {
    return syntaxError("JSON5: invalid end of input at " + q + ":" + V);
  }
  function invalidIdentifier() {
    return syntaxError("JSON5: invalid identifier character at " + q + ":" + (V -= 5));
  }
  ...
  var K = function stringify(u, D, t) {
    ...
    function serializeKey(u) {
      ...
      if (!O.isIdStartChar(D)) {
        return quoteString(u);
      }
      for (var t = D.length; t < u.length; t++) {
        if (!O.isIdContinueChar(String.fromCodePoint(u.codePointAt(t)))) {
          return quoteString(u);
        }
      }
      return u;
...
async function generateSchema(s, {headers: F, output: C, cwd: c = process.cwd()}) {
  var A = i.load({
    origin: F ? {
      url: s,
      headers: F
  ...
  var f;
  try {
    f = await A.loadSchema();
  } catch (u) {
    console.error("Something went wrong while trying to load the schema.", u);
    return;
  }
  if (!f) {
  ...
  if (!B) {
    var h;
    try {
      h = await u.readFile("tsconfig.json", "utf-8");
    } catch (u) {
      console.error("Failed to read tsconfig.json in current working directory.", u);
      return;
    }
    var p;
    try {
    ...
    var v = getGraphQLSPConfig(p);
    if (!v) {
      console.error('Could not find a "@0no-co/graphqlsp" plugin in your tsconfig.');
      return;
    } else if ("string" != typeof v.schema || !v.schema.endsWith(".graphql")) {
      console.error(`Found "${v.schema}" which is not a path to a .graphql SDL file.`);
      return;
    } else {
      B = v.schema;
    }
  }
  await u.writeFile(D.resolve(c, B), t.printSchema(f), "utf-8");
}

async function generateTadaTypes(t = !1, F = process.cwd()) {
  var C = D.resolve(F, "tsconfig.json");
  var c = await i.resolveTypeScriptRootDir(C) || F;
  var A;
  try {
    var f = D.resolve(c, "tsconfig.json");
    A = await u.readFile(f, "utf-8");
  } catch (u) {
    console.error("Failed to read tsconfig.json in current working directory.", u);
    return;
  }
  ...
  var h = getGraphQLSPConfig(B);
  if (!h) {
    console.error('Could not find a "@0no-co/graphqlsp" plugin in your tsconfig.');
    return;
  }
  return await async function ensureTadaIntrospection(u, t, F = process.cwd(), C = !0) {
    var c = i.load({
      origin: u,
      rootPath: F
    });
...
exports.default = async function main() {
  f.command("generate-schema <target>").describe("Generate a GraphQL schema from a URL or introspection file, this will be generated from the parameters to this command.").option("--header", "Pass a header to be used when fetching the introspection.").option("--output", 'A specialised location to output the schema to, by default we\'ll output the schema to the "schema" defined in your "tsconfig".').example("generate-schema https://example.com --header 'Authorization: Bearer token'").example("generate-schema ./introspection.json").action((async (u, D) => {
    ...
    return generateSchema(u, {
      headers: Object.keys(t).length ? t : void 0,
      output: D.output
    });
